...
function PascalToSnake(str) {
    var fixedStr = str.replace(/[A-Z]/g, function (letter) { return "_".concat(letter.toLowerCase()); });
...
function isLowercaseEnumsType(typeName) {
    var lowercaseEnums = SUPPORTED_ENUMS.map(function (e) { return e.toLowerCase(); });
    return lowercaseEnums.includes(typeName);
}
function validateSupportedEnum(typeName) {
    var enumIndex = SUPPORTED_ENUMS.findIndex(function (e) { return e.toLowerCase() === typeName.toLowerCase(); });
    var isSupportedEnum = enumIndex !== -1;
...
function parseLine(line, persistentData, fixedText) {
    ...
    // Add the @@map to the table name for the model
        ...
        var snakeModelName = PascalToSnake(persistentData.currentModelName);
        if (line.match(/\s+@@/)) {
            fixedText.push("  @@map(\"".concat(snakeModelName, "\")"));
            persistentData.hasAddedModelMap = true;
        }
        if (line === '}' || line === '}\r') {
            fixedText.push("\n  @@map(\"".concat(snakeModelName, "\")"));
            fixedText.push("}");
            persistentData.hasAddedModelMap = true;
            return;
        }
    ...
    if (fieldTypeMatch) {
        ...
        var _a = validateSupportedEnum(fixedFieldType), isSupportedEnum = _a[0], enumIndex = _a[1];
        if (isSupportedEnum) {
    ...
    if (relationFieldsMatch) {
        var fields = relationFieldsMatch[1].replace(/\s/g, '');
        fixedLine = fixedLine.replace(fields, fixFieldsArrayString(fields));
        //check again cos replace doesn't replace all
        var relationFieldsMatch2 = fixedLine.match(/fields:\s\[([\w,\s]+)\]/);
        if (relationFieldsMatch2) {
            var fields_1 = relationFieldsMatch2[1].replace(/\s/g, '');
            fixedLine = fixedLine.replace(fields_1, fixFieldsArrayString(fields_1));
            var relationFieldsMatch3 = fixedLine.match(/fields:\s\[([\w,\s]+)\]/);
            if (relationFieldsMatch3) {
                var fields_2 = relationFieldsMatch3[1].replace(/\s/g, '');
                fixedLine = fixedLine.replace(fields_2, fixFieldsArrayString(fields_2));
            }
        }
    ...
    if (fixedLine.includes('@relation("')) {
        ...
        var relationName = fixedLine.match(/@relation\("(\w+)"/)[1];
        //check if relation name is in camel case
        if (relationName.includes('_')) {
            //convert to camel case
            var camelCaseRelationName = snakeToCamel(relationName);
    ...
    var enumMatch = fixedLine.match(/^enum\s+(\S+)\s*\{/);
    if (enumMatch) {
        var _b = validateSupportedEnum(enumMatch[1]), isSupportedEnum = _b[0], enumIndex = _b[1];
        if (isSupportedEnum) {
